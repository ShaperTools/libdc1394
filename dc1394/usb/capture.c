...
const int PROTO_PADDING = 131072;//512;	// ARR 2013. Because occasionally we have overflows, and we don't want them to corrupt either the transfers or the other buffers.
// ARR 2013/11: Increasing padding to see if it stops the crashes at 30 minute intervals.
...
callback (struct libusb_transfer * transfer)
{
    ...
    if (transfer->status == LIBUSB_TRANSFER_CANCELLED) {
        dc1394_log_warning ("usb: Bulk transfer %d cancelled", f->frame.id);	// ARR 2013: error -> warning, I think
        return;
    }

    int status = BUFFER_FILLED;
	if(transfer->actual_length != transfer->length)	// ARR 2013
	{
		if (transfer->actual_length < transfer->length)
			status = BUFFER_CORRUPT;

		// ARR 2013
		dc1394_log_warning ("usb: Bulk transfer %d complete. Abnormal # of bytes: %d of %d bytes (%d bytes padding)",
				f->frame.id, transfer->actual_length, transfer->length, PROTO_PADDING);
	}

    if (transfer->status != LIBUSB_TRANSFER_COMPLETED) {
		// ARR 2013
		if(transfer->status == LIBUSB_TRANSFER_ERROR)
			dc1394_log_warning("usb: Bulk transfer failed with code LIBUSB_TRANSFER_ERROR");
		else if(transfer->status == LIBUSB_TRANSFER_TIMED_OUT)
			dc1394_log_warning("usb: Bulk transfer failed with code LIBUSB_TRANSFER_TIMED_OUT");
		else if(transfer->status == LIBUSB_TRANSFER_STALL)
			dc1394_log_warning("usb: Bulk transfer failed with code LIBUSB_TRANSFER_STALL");
		else if(transfer->status == LIBUSB_TRANSFER_NO_DEVICE)
			dc1394_log_warning("usb: Bulk transfer failed with code LIBUSB_TRANSFER_NO_DEVICE");
		else if(transfer->status == LIBUSB_TRANSFER_OVERFLOW)
			dc1394_log_warning("usb: Bulk transfer failed with code LIBUSB_TRANSFER_OVERFLOW");
		else
			dc1394_log_warning("usb: Bulk transfer failed with unrecognized error code");
		status = BUFFER_CORRUPT;
...
init_frame(platform_camera_t *craw, int index, dc1394video_frame_t *proto)
{
    ...
    f->frame.image = craw->buffer + index * (proto->total_bytes + PROTO_PADDING);	// ARR 2013
    f->frame.id = index;
        ...
        uint32_t flags)
{
    ...
    craw->frames_ready = 0;
    craw->queue_broken = 0;
    craw->buffer_size = proto.total_bytes * num_dma_buffers + PROTO_PADDING * num_dma_buffers;	// ARR 2013
